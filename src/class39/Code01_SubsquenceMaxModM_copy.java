...
public class Code01_SubsquenceMaxModM_copy {
	...
	public static int maxA(int[] arr, int m) {
		HashSet<Integer> set = new HashSet<>();
		processA(arr, 0, 0, set);
		int max = 0;
		for (Integer i : set) {
	...
	private static void processA(int[] arr, int index, int sum, HashSet<Integer> set) {
		if (index == arr.length) {
			set.add(sum);
		} else {
			process(arr, index + 1, sum, set);
	...
	public static int maxB(int arr[], int m) {
		int sum = 0;
		for (int i : arr) {
			sum += i;
		}
