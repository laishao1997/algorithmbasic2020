...
public class Code02_SnacksWaysMain {
    ...
    public static long getWays(int arr[], int bag) {
        if (arr == null || arr.length == 0) {
            return 0;
        }
        if (arr.length == 1) {
        ...
        int mid = (arr.length - 1) >> 1;
        TreeMap<Long, Long> lmap = new TreeMap<>();
		//货物来自左侧或者货物来自于右侧或者同时来自于左和右，所以process方法有返回值
        long ways = processWays(arr, 0, mid, 0, bag, lmap);
        TreeMap<Long, Long> rmap = new TreeMap<>();
        ways += processWays(arr, mid + 1, arr.length - 1, 0, bag, rmap);
        TreeMap<Long, Long> rpre = new TreeMap<>();
        long pre = 0;
		for (Entry<Long, Long> entry : rmap.entrySet()) {
			pre += entry.getValue();
    ...
    public static long processWays(int arr[], int index, int end, long w, long bag, TreeMap<Long, Long> map) {
        if (w > bag) {
            return 0;
        }
        if (index > end) {
            //这里相当于把0这种方式放到了方法外层进行处理
            if (w != 0) {
            ...
            long ways = processWays(arr, index + 1, end, w, bag, map);
            ways += processWays(arr, index + 1, end, w + arr[index], bag, map);
            return ways;
        }
    }
