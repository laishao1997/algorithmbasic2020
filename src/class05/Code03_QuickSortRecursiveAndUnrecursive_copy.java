...
public class Code03_QuickSortRecursiveAndUnrecursive_copy {
    ...
    public static void qs(int[] arr) {
        ...
        ps(arr, 0, arr.length - 1);
    }


    public static void ps(int[] arr, int L, int R){
        if (L >= R) {
            return;
        }
        int rand = L + (int) (Math.random() * (R - L + 1));
        swap(arr, rand, R);
        int[] equals = lands(arr, L, R);
        ps(arr, L, equals[0] - 1);
        ps(arr, equals[1] + 1, R);
    }

    private static int[] lands(int[] arr, int L, int R) {
        if (L > R) {
            return new int[]{-1, -1};
        }
        if (L == R) {
